...
|---|---|---|---|
| T-001 | Inisialisasi monorepo pnpm workspaces (`apps/web`, `apps/api`, `packages/shared`) | 05 §1 | `[ ]` |
| T-002 | Setup `apps/web`: Vite 6 + React 19 + TypeScript strict + Tailwind v4 | 05 §2, §4.4 | `[ ]` |
...
| T-004 | Setup `apps/api`: NestJS 11 + Prisma 6 + PostgreSQL via docker-compose | 05 §1.2 | `[ ]` |
| T-005 | Setup `packages/shared`: enum, konstanta, skema Zod kosong siap diisi | 05 §1, 03 §2 | `[ ]` |
| T-006 | Konfigurasi ESLint, Prettier, Husky, lint-staged, commitlint | 05 §9 | `[ ]` |
| T-007 | Setup CI GitHub Actions: lint, typecheck, test, build | 05 §9 | `[ ]` |
| T-008 | Buat `.env.example` untuk web dan api; validasi env dengan Zod di backend | 05 §11, §5.5 | `[ ]` |
...
| T-011 | Kunci semua enum & daftar nilai pilihan di `packages/shared/src/constants.ts` | 03 §2, 04 §5 | `[ ]` |
| T-012 | Tulis label Bahasa Indonesia untuk setiap nilai enum di `packages/shared/src/labels.ts` | 08 §7 | `[ ]` |
